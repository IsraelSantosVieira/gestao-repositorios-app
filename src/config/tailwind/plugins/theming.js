...
// -----------------------------------------------------------------------------------------------------
// @ repositorioApp TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--repositorioApp-${e(key)}`, value], [`--repositorioApp-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            ]))
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`repositorioApp.customProps.background.${colorScheme}`);
            const foreground = theme(`repositorioApp.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--repositorioApp-${e(key)}`, value], [`--repositorioApp-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--repositorioApp-${e(key)}`, value], [`--repositorioApp-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--repositorioApp-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--repositorioApp-on-${name}-rgb), <alpha-value>)`]
                    ])))
                },
                repositorioApp  : {
                    customProps: {
